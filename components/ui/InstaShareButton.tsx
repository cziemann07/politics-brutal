    ...
    <>
      ...
      <div style={{ 
        ...
        <div ref={ref}
             ...
             style={{ 
                width: '1080px',
                height: calculateDynamicHeight() // Função dinâmica!
             }}
        >
          ...
          <div className="mb-8 w-full">
            ...
            {/* FORCEI whitespace-nowrap aqui também para o título principal não quebrar "OS" */}
            ...
            <div className="flex justify-end items-center mt-0">
                <div className="bg-black text-white px-6 py-2 flex gap-8">
                    ...
                    <p className="font-mono text-lg uppercase opacity-80">DADOS: {data.dataDate}</p>
                </div>
            </div>
          </div>
          ...
          <section className="mb-12 w-full">
            {/* Adicionei whitespace-nowrap para impedir "O" numa linha e "JOGO" noutra */}
                ...
                {data.timelineSteps.map((step) => (
                    <div key={step.id} className="flex items-start gap-5 relative">
                        ...
                        <div className="text-2xl leading-tight pt-1">
                            ...
                            {step.textPrefix} <span className="text-brutal-red font-black">{step.highlight}</span> {step.textSuffix}
                        </div>
                    </div>
                ))}
            </div>
          ...
          {/* === SEÇÃO 2: FAMÍLIA (O PROBLEMA ESTAVA AQUI) === */}
          <section className="mt-8 mb-10 w-full">
            {/* CORREÇÃO: 
                1. Mudei items-end para items-center (alinha melhor o ícone).
                2. Adicionei whitespace-nowrap no h2 (Impede a quebra de linha).
                3. Tirei leading-none do container pai e coloquei margem embaixo do título.
            */}
            ...
            <div className="grid grid-cols-2 gap-8">
                {data.familyCards.map((card, index) => (
                    <div key={index} className="border-[3px] border-black p-6 bg-white shadow-hard flex flex-col justify-between h-full">
                        <div>
                            <div className="flex justify-between items-start mb-2">
                                <h3 className="font-black text-3xl uppercase leading-[0.9] max-w-[80%]">
                                    {card.name}
                                </h3>
                                <span className={`text-xs font-black px-2 py-1 uppercase border border-black shrink-0 ${card.roleBadgeColor === 'red' ? 'bg-brutal-red text-white' : 'bg-gray-200 text-black'}`}>
                                    {card.roleBadge}
                                </span>
                            </div>
                        ...
                        <div className="border-t-2 border-dashed border-gray-400 pt-3 mt-auto">
                            <span className="text-sm font-bold block uppercase text-gray-500">{card.detailLabel}</span>
                            <span className="font-black text-2xl">{card.detailValue}</span>
                        </div>
                    </div>
                ))}
            </div>
